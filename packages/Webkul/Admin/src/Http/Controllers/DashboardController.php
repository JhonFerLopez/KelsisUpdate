...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $statistics = [
            ...
            'top_selling_preparations'   => $this->getTopSellingPreparations(),
    ...
    /**
     * Returns the list of top selling preparations.
     *
     * @return \Illuminate\Support\Collection
     */
    public function getTopSellingPreparations()
    {
        return $this->orderItemRepository->getModel()
            ->leftJoin('products', 'order_items.product_id', 'products.id')
            ->leftJoin('product_preparations', 'products.id', 'product_preparations.product_id')
            ->leftJoin('preparations', 'product_preparations.preparation_id', 'preparations.id')
            ->leftJoin('preparation_translations', 'preparations.id', 'preparation_translations.preparation_id')
            ->where('preparation_translations.locale', app()->getLocale())
            ->where('order_items.created_at', '>=', $this->startDate)
            ->where('order_items.created_at', '<=', $this->endDate)
            ->addSelect(DB::raw('SUM(qty_invoiced - qty_refunded) as total_qty_invoiced'))
            ->addSelect(DB::raw('COUNT(' . DB::getTablePrefix() . 'products.id) as total_products'))
            ->addSelect('order_items.id', 'preparations.id as preparation_id', 'preparation_translations.name')
